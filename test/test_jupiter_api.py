...
class TestCreateLimitOrder(TestBase):
    ...
    @patch("requests.post")
    def test_create_limit_order_correct_http_method(
        self, mock_post, mock_wallet, mock_response
    ):
        mock_post.return_value = mock_response
...
class TestCancelOrders(TestBase):
    @patch("requests.post")
    def test_cancel_orders_success(self, mock_post):
        ...
        result = self.api.cancel_orders(maker="maker_address")
    ...
    @patch("requests.post")
    def test_cancel_orders_failure(self, mock_post):
        mock_response = MagicMock()
        mock_response.status_code = 400
        mock_response.json.return_value = {"error": "Cancellation failed"}

        with pytest.raises(Exception) as exc_info:
            self.api.cancel_orders(maker="maker_address")

        assert "Cancelation failed" in str(exc_info.value)
        mock_post.assert_called_once()
    ...
    def test_cancel_orders_correct_endpoint_url_and_http_method(self):
        expected_url = "https://api.jup.ag/limit/v2/cancelOrders"
        pass
